...
- `handleTrusted` / `handleTrustedWithEvent` from `./trusted-ipc` are the only registration
  ...
  scan can see.
- Parse every argument. The renderer sends `unknown`: `./ipc/validation.ts` has `requireString`,
  `isObject` and `optionalBoolean`, and the `*-inputs.ts` files hold the per-domain parsers. A
  handler that reads `input.path` off an untyped value is the boundary failing open.
