...
describe("conversation keys", () => {
  it("keys drafts by server and bot", () => {
    expect(composerDraftKey({ botId: "chief", serverId: "local" })).toBe("chief");
    expect(composerDraftKey({ botId: "chief", serverId: "team-1" })).toBe("team-1:chief");
    expect(composerDraftKey({ botId: "chief", serverId: "team-1" })).not.toBe(
      composerDraftKey({ botId: "chief", serverId: "team-2" }),
    );
  });
