...
import type { BotMemory, BotMemoryOrigin } from "@openbot/contracts/ipc";
...
export class AgentMemoryStore {
  ...
  get(botId: string, memoryId: string): BotMemory | null {
  ...
  createManual(botId: string, text: string): BotMemory {
  ...
  saveAutomatic(input: SaveAutomaticMemoryInput): BotMemory | null {
    if (input.memoryId && input.expectedUpdatedAt !== undefined) {
      const current = this.get(input.botId, input.memoryId);
  ...
  delete(botId: string, memoryId: string, expectedUpdatedAt?: string | null): boolean {
    const current = this.get(botId, memoryId);
    ...
    this.database.dispatch(
      ...
      (db, sequences) => {
        ...
        db.prepare("DELETE FROM projection_agent_memories WHERE agent_id = ? AND memory_id = ?").run(botId, memoryId);
        return true;
      },
    );
    return true;
  ...
  clear(botId: string): number {
    const memories = this.list(botId);
    ...
    return this.database.dispatch(
      ...
      memories.map((memory) => ({
        ...
        eventType: "agent-memory.deleted",
        payload: { botId, memoryId: memory.id },
      })),
      (db, sequences) => {
  ...
  #save(input: {
    ...
    const duplicate = this.#findByNormalizedText(input.botId, normalizedText);
    if (duplicate && duplicate.id !== input.memoryId) {
      if (input.memoryId) this.delete(input.botId, input.memoryId);
      return duplicate;
    }

    const previous = input.memoryId ? this.get(input.botId, input.memoryId) : null;
    if (input.memoryId && !previous) throw new Error("This memory no longer exists.");
    if (!previous && this.list(input.botId).length >= INPUT_LIMITS.agentMemories) {
  ...
  #findByNormalizedText(botId: string, normalizedText: string): BotMemory | null {
    const row = databaseRow(
      this.database.connection
        .prepare(
          `SELECT memory_id, agent_id, text, origin, source_turn_id, created_at, updated_at
...
function memoryFromRow(row: DynamicRecord): BotMemory {
