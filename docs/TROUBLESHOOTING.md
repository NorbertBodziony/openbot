...
## OpenBot says Codex setup is required
...
Then run `codex login`, sign in with ChatGPT, fully quit OpenBot, and open it again. OpenBot requires
Codex CLI 0.144.1 or newer and a local ChatGPT login; it does not accept an API key.
...
remove `~/.codex` unless you intentionally want to manage Codex CLI's own login and local history.

## Uninstall

Quit OpenBot and remove `OpenBot.app` from Applications. If you also want to remove local OpenBot
data, follow the reset steps above. Codex CLI and its data are independent and are not removed with
OpenBot.

## Report a problem

Use [GitHub Issues](https://github.com/NorbertBodziony/openbot/issues) for reproducible bugs. Include
the OpenBot version, macOS version, Apple Silicon model, Codex CLI version, and minimal reproduction
steps. Never publish tokens, `~/.codex`, conversations, private files, Electron user data, or full
