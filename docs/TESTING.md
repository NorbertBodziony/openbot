...
| Biome + 12 GritQL rules | `biome.json`, `tools/biome/anti-slop/rules/` | syntactic rules, unsafe assertions, module mocking |
...
| vitest project `node` | `src/backend`, `src/main`, `src/preload`, `scripts`, `packages/contracts` | **the default.** Anything separable from the DOM |
| vitest project `renderer` (jsdom) | `src/renderer`, `packages/brand` | only component behaviour that cannot be extracted |
...
Project boundaries are defined in `vitest.config.ts`. A test placed in the wrong
project either loses its environment or silently drags in jsdom.
...
10. **Pure-logic test** — projections, reducers, parsers, state machines, schedulers.
    Runs in the `node` project **even when the module lives in the renderer.** The
    cheapest tests in the repository and the best ratio of value to brittleness.
