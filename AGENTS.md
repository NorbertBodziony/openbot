...
- Put pure logic in the vitest `node` project even when the module lives in the renderer. The project is chosen by file extension: `src/renderer/**/*.test.ts` runs in `node`, `*.test.tsx` in jsdom. A logic test that needs jsdom is a signal the logic is not separable yet.
- The `src/renderer/src/App*.test.tsx` files are closed to new cases. New coverage goes to a module or to a component test at the lowest stable boundary.
