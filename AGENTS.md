...
- The default answer is no test. Prefer changing an existing test over adding one. A new test is an exception that must name the consequence it protects and the boundary you checked. A new test file needs a boundary that does not exist yet.
- Before writing a test, check whether `tsc`, Biome and its GritQL rules, `bun run check:ui`, `bun run check:tests`, or Storybook already enforces the rule. If one of them does, do not write the test.
...
- Put pure logic in the vitest `node` project even when the module lives in the renderer. The file name picks the project: `src/renderer/**/*.test.ts` runs in `node` without a DOM, `src/renderer/**/*.test.tsx` renders JSX and gets jsdom, and `*.dom.test.ts` is the narrow case of needing a DOM without rendering a component. Needing either of the last two for a logic test is a signal the logic is not separable yet.
- The `src/renderer/src/App*.test.tsx` files are closed to new cases: `bun run check:tests` records a case count per file and those counts may only decrease, so a new file in the family fails too. New coverage goes to a module or to a component test at the lowest stable boundary.
- `bun run test` runs the desktop suite, `apps/auth-api` and `apps/site-router`. Use `bun run test:desktop -- <path>` for a single desktop file; do not report a change as tested after running only part of that.
- `bun run check:tests` enforces the mechanical half of this policy. Its budgets are recorded per file and may only decrease, so existing debt cannot be traded between files, and it also verifies that this section and `docs/TESTING.md` still describe the mechanisms that exist. Reaching for a class-name `querySelector`, `document.activeElement`, a DOM walk inside `expect`, or a `data-testid` the test did not render itself means the rule belongs at a different boundary.
