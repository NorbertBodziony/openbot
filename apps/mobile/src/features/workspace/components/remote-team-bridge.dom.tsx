...
import {
  ...
  signEd25519,
  verifyEd25519Pem,
} from "@openbot/team-client/ed25519";
...
interface RemoteTeamBridgeProps {
...
export default function RemoteTeamBridge({
  ...
  const actions = useRef({ getBootstrap, endSession, onConnectionUpdate, onTeamEvent });
  actions.current = { getBootstrap, endSession, onConnectionUpdate, onTeamEvent };

  useEffect(() => {
    if (!command || processedCommandId.current === command.id) return;
    processedCommandId.current = command.id;
    void executeCommand(command, actions).then(onCommandResult);
...
async function executeCommand(
  ...
  actions: RefObject<Pick<RemoteTeamBridgeProps, "getBootstrap" | "endSession" | "onConnectionUpdate" | "onTeamEvent">>,
): Promise<RemoteTeamCommandResult> {
  try {
    if (command.type === "connect") {
      if (peer?.authenticated && peer.hostId === command.hostId) return { commandId: command.id, ok: true };
...
async function connectPeer(
  ...
  const connected = new Promise<void>((resolve, reject) => {
    ...
    state.connectedTimer = window.setTimeout(
...
function openSignal(
  ...
  if (state.closed || peer !== state || state.socket) return;
...
function createPeerConnection(
  ...
  connection.onconnectionstatechange = () => {
    ...
    if (connection.connectionState === "failed")
      void restartIce(state).catch((error) => failPeer(state, error, actions));
  };
  return connection;
}
...
async function request(method: string, path: string, body: TeamProtocolV2Json) {
  const state = peer;
  if (!state?.authenticated) throw new Error("The selected server is offline.");
